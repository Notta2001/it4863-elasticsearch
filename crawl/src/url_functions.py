...
def get_content_news_from_news_url(url: str) -> dict:
    ...
    try:
        ...
        image_url =  soup.find('div', {'class': 'fig-picture'}).find('img')['data-src']

        date = soup.find("span", class_="date").text
        date_list = date.split(',')
        date_str = date_list[1]
        time_str = date_list[2].split(' ')[1]
        date_time = datetime.strptime(date_str + " " + time_str, f' %d/%m/%Y %H:%M')
        tuple = date_time.timetuple()
...
def get_page_urls_from_sub_topic_url(sub_topic_url, pages=10) -> list:
...
def get_all_news_urls_from_topics_links(topics_links: dict, n_pages_per_topic=10) -> dict:
