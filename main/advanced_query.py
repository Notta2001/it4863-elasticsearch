...
def build_query(normal=None, title_query=None, content_query=None, des_query=None, minimum_should_match=0, analyzer="my_analyzer"):
    ...
    must_not_query = []
    if normal:
        should_query.append({"match": {'title': {"query": normal, "analyzer": analyzer}}})
        should_query.append({"match": {'content': {"query": normal, "analyzer": analyzer}}})
        should_query.append({"match": {'description': {"query": normal, "analyzer": analyzer}}})

    if title_query:
        if title_query['match']:
            should_query.append({"match": {'title': {"query": title_query['match'], "boost": title_query['boosting']}}})
        if title_query['any']:
            for value in title_query['any']: 
                should_query.append({"match": {'title': {"query": value, "boost": title_query['boosting']}}})
        if title_query['term']:
    ...
    if content_query:
        if content_query['match']:
            should_query.append({"match": {'content': {"query": content_query['match'], "boost": content_query['boosting']}}})
        if content_query['any']:
            for value in content_query['any']: 
                should_query.append({"match": {'content': {"query": value, "boost": content_query['boosting']}}})
    ...
    if des_query:
        ...
        if des_query['any']:
            for value in des_query['any']: 
                should_query.append({"match": {'description': {"query": value, "boost": des_query['boosting']}}})
        if des_query['term']:
    ...
    #     s = s.query("function_score", should=should_query, must=must_query, must_not=must_not_query, minimum_should_match=0, query=s.to_dict(), functions=[{"script_score": {"script": {"source": "1.0 + ln(doc['title'].value + 1.0)"}}}])
    # elif scoring_function == "bm25":
    #     s = s.query("function_score", should=should_query, must=must_query, must_not=must_not_query, minimum_should_match=0, query=s.to_dict(), functions=[{"bm25": {"boost_mode": "multiply", "field": "title"}}])
